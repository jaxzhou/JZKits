...
@implementation NSDate (ChineseLunar)

-(NSString*)lunarDate{
    NSCalendar *localeCalendar = [[NSCalendar alloc] initWithCalendarIdentifier:NSChineseCalendar];
    unsigned unitFlags = NSYearCalendarUnit | NSMonthCalendarUnit |  NSDayCalendarUnit;
    NSDateComponents *localeComp = [localeCalendar components:unitFlags fromDate:self];
    NSArray *chinesemonth = [NSArray arrayWithObjects:@"正月",@"二月", @"三月", @"四月", @"五月", @"六月", @"七月", @"八月",@"九月",@"十月",@"十一月",@"十二月", nil];
    NSArray *chineseday = [NSArray arrayWithObjects:@"初一",@"初二", @"初三", @"初四", @"初五", @"初六", @"初七", @"初八",@"初九",@"初十",@"十一",@"十二", @"十三",@"十四",@"十五",@"十六",@"十七",@"十八",@"十九",@"二十",@"廿一",@"廿二",@"廿三",@"廿四",@"廿五",@"廿六",@"廿七",@"廿八",@"廿九",@"三十",nil];
    return [NSString stringWithFormat:@"%@%@",chinesemonth[localeComp.month-1],chineseday[localeComp.day-1]];
}
...
-(NSString*)lunarSpecialDay{
    //get chinese date components
    NSDateComponents *componets = [[NSCalendar currentCalendar] components:NSYearCalendarUnit|NSMonthCalendarUnit|NSDayCalendarUnit fromDate:self];
    ...
    NSInteger array_index = (componets.year - START_YEAR)*12+componets.month -1 ;
    ...
    NSInteger index = -1;
    if(componets.day == day){
        index = (componets.month-1) *2 + (componets.day>15? 1: 0);
    }
...
+(NSDate*)dateOfLunar:(NSString*)lunar atYear:(int)year{
    NSArray *chineseDays=[NSArray arrayWithObjects:
                          ...
                          @"寒露", @"霜降", @"立冬", @"小雪", @"大雪", @"冬至",nil];
    NSInteger i = [chineseDays indexOfObject:lunar];
    NSInteger m = i / 2 + 1;
    NSInteger arrayIndex = (year - START_YEAR)*12 + m - 1;
    int64_t flag = gLunarHolDay[arrayIndex];
    int64_t day = 15-((flag>>4)&0x0f);
    ...
    componts.month = m;
    componts.day = day;
    NSDate *d = [[NSCalendar currentCalendar] dateFromComponents:componts];
    
    if (![[d lunarSpecialDay] isEqualToString:lunar]) {
        m = (i-1)/2+1;
        arrayIndex = (year - START_YEAR)*12 + m - 1;
        flag = gLunarHolDay[arrayIndex];
        day = ((flag)&0x0f)+15;
        componts.year = year;
        componts.month = m;
        componts.day = day;
        d = [[NSCalendar currentCalendar] dateFromComponents:componts];
    }
    return d;
}
...
+(NSString*)nextLunar:(NSString*)lunar{
    NSArray *chineseDays=[NSArray arrayWithObjects:
                          ...
                          @"寒露", @"霜降", @"立冬", @"小雪", @"大雪", @"冬至",nil];
    NSInteger index = [chineseDays indexOfObject:lunar];
    if (index!=NSNotFound) {
        index += 1;
        if (index==[chineseDays count]) {
            index = 0;
        }
        return [chineseDays objectAtIndex:index];
    }
